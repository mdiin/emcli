...
All nodes except `:canvas`, `:divider` and the action and navigation tags `:button`, `:icon-button` and `:link` accept `field-name` (string) and `command-input` (bool) in addition to their own attrs listed below.
