...
Start by validating the model. If it errors (server not reachable), guide the user to start one before continuing:
...
Ask them to start it in a separate terminal, then retry validation before proceeding.
...
Figure out if the model is empty by having an `em-explorer` resolve and query for elements and timelines; the presence of either indicates an existing model.
...
2. Validate the model to find spots that need attention
